...
- [ ] **Portfolio CIV truncates at the earliest-ending asset's last date** — surfaced 2026-06-16 while verifying the plot fix. `port-everything` ends 2024-02-21 because the SGB data stops at the final tranche issue date (FY 2023-24 Series IV, 2024-02-21), so `min(ends)` in `combined_civ_series` truncates the whole portfolio CIV ~16 months short of "today". Now we know *why* SGB stops there (scheme discontinued in Union Budget 2025); the right fix is part of the SGB modeling refactor below — once SGB is modeled as a per-tranche holding driven by IBJA gold spot, the SGB CIV extends to today and the portfolio truncation disappears.
...
- [ ] **SGB modeling refactor — Phase 2: integration** (2026-06-16, Phase 1 done; this is the follow-up). With `sgb_holdings.sgb_holding_civ` available, the next cycle wires it through. Required changes: (a) portfolio TOML schema accepts a list `[[sgb]]` of `{tranche_id, units_grams}` entries per the "different tranches → different investments; same-tranche-same-day → lump" rule; (b) `main.py` iterates those entries, calls `sgb_holding_civ` per tranche with `load_gold_prices_per_gram()` as the gold input, and registers each tranche as a separate asset in `PortfolioTimeseries`; (c) `sgb_loader.create_sgb_daily_returns` (the bogus `pct_change`-on-consecutive-issue-prices function) is deleted; (d) goldens re-captured. Holdings TOML stays out of the public repo (personal data); the existing `port-everything.toml` will be migrated to use sample tranches for the canonical golden.
