...
- [ ] **Investigate enormous `alpha_capm` on low-beta portfolios** (surfaced 2026-06-16). `port-mf-ppf-gold` reports Alpha = 68.09% at Beta 0.35; `port-everything` after the Phase 2 SGB fix reports Alpha = 139.15% at Beta 0.10. `port-1` (pure-MF, no gold/SGB) reports Alpha = 4.13% at Beta 0.50 which is plausible. Pattern: as beta drops toward 0 (portfolio decorrelates from NIFTY), reported alpha balloons. Likely a per-period-vs-annualized units mismatch in `TimeseriesReturn.alpha_capm` — the residual may not be being annualized while the inputs implicitly are. Not blocking, but it's currently the most prominently-wrong number on the plot.
