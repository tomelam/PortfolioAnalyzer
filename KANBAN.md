...
- [ ] Investigate remaining implausibly large daily-method Sharpe/Vol on synthetic-CIV portfolios — after the `combined_civ_series` normalization fix (below), port-mf-ppf-gold daily Sharpe=4.98 / Vol=48% and port-everything daily Sharpe=7.33 / Vol=33%. Monthly figures are reasonable (0.44 and 0.65). Remaining cause: forward-filled synthetic CIVs produce zero-return days that distort the variance denominator. Likely fix: `metrics.volatility` accepts an `exclude_zero_return_days` flag, or upstream the forward-fill to use NaN instead of zero so `pct_change().dropna()` filters them.
...
- [ ] **13 skip-marked tests still rely on `TimeseriesReturn(returns)` construction** — the class still treats `_series` as a price/CIV (applies `pct_change` internally). Pure-function alternatives in `metrics.py` cover the same math directly. To unblock the legacy tests cleanly, either: (a) add a `TimeseriesReturn.from_returns(series)` alt-constructor that skips `pct_change`, or (b) rewrite the tests to call `metrics.sharpe/sortino/volatility` directly. Option (b) is now trivial.
