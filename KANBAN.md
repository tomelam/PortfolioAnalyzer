...
- [ ] Investigate implausibly large daily-method Sharpe/Vol on synthetic-CIV portfolios — port-mf-ppf-gold daily Sharpe=7.25, Vol=52%; port-everything daily Sharpe=4.99, Vol=37.76%. Monthly figures look reasonable (0.96 and 0.27). Likely cause: forward-filled synthetic CIVs produce zero-return days that distort the variance denominator. **Phase D progress:** `metrics.py` extracted with pure functions and 20 hand-verified unit tests; `TimeseriesReturn.{sharpe,sortino,volatility,cagr,max_drawdown,max_drawdowns}` now delegate. The actual synthetic-CIV variance-distortion fix is now isolatable in pure-function form — likely needs `metrics.volatility` to accept an `exclude_zero_return_days` flag, or upstream the forward-fill to use NaN instead of zero so `pct_change().dropna()` filters them.
