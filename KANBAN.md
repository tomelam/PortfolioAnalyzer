...
- [ ] **SGB modeling refactor** (deeper, separate cycle). `sgb_loader.py` currently treats consecutive tranches' *issue* prices as if they were a daily price history for a single bond and runs `pct_change` on them — which is wrong: the issue price is the launch price of each tranche, not a mark-to-market quote. Proper modeling needs: (a) per-tranche P&L driven by IBJA gold spot (price appreciation against the redemption formula) + (b) semi-annual coupon accrual at 2.5% on the nominal value + (c) status transitions LOCK → PRE → MAT based on issue_date + 5y / +8y. The new `sgb_tranches.py` reference data is the foundation; the per-tranche valuation engine is the next cycle. Holdings as input (which tranches, how many grams) should come from a TOML config that is *not* checked into the public repo (personal data).
