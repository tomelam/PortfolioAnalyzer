...
- [ ] Investigate implausibly large daily-method Sharpe/Vol on synthetic-CIV portfolios — port-mf-ppf-gold daily Sharpe=7.25, Vol=52%; port-everything daily Sharpe=4.99, Vol=37.76%. Monthly figures look reasonable (0.96 and 0.27). Likely cause: forward-filled synthetic CIVs produce zero-return days that distort the variance denominator. Phase D investigation; the golden currently pins this (incorrect-looking) behavior. **Confirmed evidence:** 13 of the pre-existing `test_timeseries.py` / `test_timeseries_errors.py` tests fail with NaN-or-wrong Sharpe/Sortino/Volatility on small known series; now skip-marked with this reason, will turn green once the metric layer is extracted with proper TDD.
- [ ] **Reimplement `max_drawdowns(threshold)`** — the method is commented out as a docstring at `timeseries.py:309`. 4 pre-existing tests (`test_max_drawdowns_basic`, `_behavioral`, `_no_recovery`, `_none`) document the expected contract: "list of (peak_date, trough_date, drawdown_pct, recovery_days) for drawdowns whose magnitude exceeds threshold and which have fully recovered to the prior peak." Currently skip-marked.
