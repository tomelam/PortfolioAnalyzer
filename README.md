...
This repository contains the Portfolio Analyzer application. It fetches historical NAV data for Indian mutual funds from [mfapi.in](https://mfapi.in), uses benchmark data from [investing.com](https://investing.com) or [niftyindices.com](https://in.investing.com/indices/nifty-total-returns-historical-data), uses risk-free rate data from [FRED](https://fred.stlouisfed.org), [RBI](https://rbi.org.in), or [DBIE (by advanced-searching "bill" while selecting "Weekly" Report Frequency and "Publication" Function)](https://data.rbi.org.in), and uses the daily gold price (the [LBMA Gold Price PM fix](https://www.lbma.org.uk/prices-and-data/precious-metal-prices), USD per troy ounce — the canonical London free-market benchmark, fetched from the LBMA's own feed and used unconverted, since the analyzer reports only normalized returns), computes key portfolio performance metrics (such as annualized return, volatility, Sharpe/Sortino ratios, Alpha, Beta, and maximum drawdowns), and visualizes historical returns along with benchmark data.
...
- **Data Loading & Alignment:**  
  ...
  - Uses the **LBMA Gold Price** (afternoon "PM" fix, USD per troy ounce) — the canonical London free-market benchmark — auto-fetched from the LBMA's own JSON feed ([prices.lbma.org.uk](https://prices.lbma.org.uk/json/gold_pm.json)) and stored in `"data/reference/gold_lbma_usd_daily.csv"`. This is a daily, deep-history (back to 1968), no-auth public feed, refreshed automatically like the benchmark/risk-free sources (see `docs/DATA_REFRESH.md`). It replaced the dead World Gold Council "gold price averages since 1978" dataset (discontinued March 2025 when ICE pulled the historical LBMA Gold Price from third-party redistributors). The price is kept in **USD**, unconverted: the analyzer reports only normalized returns, and a time-varying USD/INR path would inject rupee/RBI dynamics into gold's measured return.
