...
| `probe_niftyindices_catalog_names.py` | Confirmed the exact TRI-endpoint spelling for the fund-catalog equity benchmarks (`data/funds/fund_catalog.csv`). One stealth session; the endpoint accepts both abbreviated (`NIFTY SMLCAP 250`) and full (`NIFTY SMALLCAP 250`) names — all 10 fetchable equity indices returned rows. |
| `probe_rbi_sgb_redemption.py` | **B2 Step 0 (fail-loud gate):** whether a fetchable, machine-readable RBI source of SGB *redemption* prices exists. Result: **PASS** on plain `requests` — the CAPTCHA-walled mobile directory (`BS_SwarnaBharat.aspx`) is routed around; `SearchResults.aspx` enumerates redemption press-release PRIDs and each `BS_PressReleaseDisplay.aspx?prid=N` yields the date + ₹/unit price (1 unit = 1 g). No IBJA paid API / stealth browser needed. |
