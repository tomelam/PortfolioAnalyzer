...
def plot_cumulative_returns(
    ...
    print("Print some sanity checks before adjusting projected")
    print(cumulative_historical.index)  # Check the entire index
    print(cumulative_historical.index[-1])  # Check the last element
    print(type(cumulative_historical.index[-1]))  # Confirm its type
    print(adjusted_projected.index)  # Check the entire index
    print(type(adjusted_projected.index))  # Confirm its type
    print(adjusted_projected.index.isnull().sum())  # Check for NaNs
    adjusted_projected.index = cumulative_historical.index[-1] + pd.to_timedelta(adjusted_projected.index, unit='D')
    #adjusted_projected += cumulative_historical.iloc[-1]

    # Plot historical and projected returns
    plt.plot(cumulative_historical * 100, label="Historical Portfolio Returns", color="blue")
    plt.plot(adjusted_projected * 100, label="Projected Portfolio Returns", color="red", linestyle="--")
    ...
    plt.axvline(cumulative_historical.index[-1], color="gray", linestyle=":", label="Projection Start")

    # Display allocations just under the legend
    if allocations:
        allocations_text = "\n".join([f"{key}: {value * 100:.2f}%" for key, value in allocations.items()])
        plt.gca().text(
            0.02, 0.68, allocations_text,  # Slightly below the legend (adjust `y` as needed)
    ...
    if metrics:
        metrics_text = "\n".join([
            ...
            f"{key}: {value:.4f}" if isinstance(value, (int, float)) else
            f"{key}: {value}"
    ...
    plt.legend(handles=handles, labels=labels, loc='upper left')  # Adjust `loc` as needed
