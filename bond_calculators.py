...
'''
def calculate_variable_bond_cumulative_gain(rate_df, portfolio_start_date, end_date=None):
    """
    Calculate a daily cumulative gain series for a bond with variable annual rates.
    Expects rate_df to have a DateTimeIndex and a column 'interest' (annual rate in percent).
    """
    if end_date is None:
        end_date = pd.Timestamp.today()
    dates = pd.date_range(start=portfolio_start_date, end=end_date, freq='B')
    # Map each day to the appropriate annual rate (using forward fill)
    daily_rates = rate_df.reindex(dates, method='ffill')['interest']
    daily_rates = daily_rates.astype(float) / 100 / 252
    cum_gain = (1 + daily_rates).cumprod()
    return cum_gain

def calculate_variable_bond_cumulative_gain(rate_df, portfolio_start_date):
    # (existing code to compute the cumulative series, say 'cum_series')
    # ...
    # Ensure the series is on a daily (business day) frequency:
