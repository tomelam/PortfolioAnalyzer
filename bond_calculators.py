...
def calculate_variable_bond_cumulative_gain(rate_df, portfolio_start_date):
    ...
    Expects:
      - rate_df: a DataFrame with a DatetimeIndex and a column 'interest' (or 'rate') representing the annual
        interest rate in percent.
      - portfolio_start_date: the start date for the calculation.

    Returns:
      pd.DataFrame: A daily cumulative gain series with column "var_rate_bond_value".
    """
    import pandas as pd

    # Standardize the rate column name
    ...
    # Use the later of portfolio_start_date and the earliest rate date.
    ...
    # Convert annual rate (in percent) to daily effective return.
